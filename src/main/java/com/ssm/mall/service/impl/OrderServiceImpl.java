...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerRes mlist(Integer page, Integer size) {
        ...
        for(Order order:orderList){
            List<Item> itemList = itemDao.selectAllByOrderNo(order.getOrderNo());
    ...
    @Override
    public ServerRes msearch(SearchVO sv, Integer page, Integer size) {
        PageHelper.startPage(page,size);
        List<Order> orderList = orderDao.msearch(sv);
        PageInfo pageInfo = new PageInfo(orderList);
        List<OrderVO> result = Lists.newArrayList();
        pageInfo.setList(result);
        for(Order order:orderList){
    ...
    ********************************portal门户功能编写******************************************
    ...
    public ServerRes createOrder(Integer userId, Integer shippingId){
        ...
        List<Cart> cartList = cartDao.selectCheckedCarts(userId);
        ...
        for(Item item:itemList){
            payment = BigDecimalUtil.add(payment,item.getTotalPrice());
        }
        //组装订单Order对象
        ...
        //插入订单
        int flag = orderDao.insertSelective(newOrder);
        if(flag < 1){
            return ServerRes.error(Result.ORDER_CREATE_FAILED);
        }
        //重新查询订单，将createTime等属性读出
        Order order = orderDao.selectByUseridAndOrderno(userId,newOrder.getOrderNo());

        //订单插入成功后，批量插入订单详情
        for(Item item:itemList){
            item.setOrderNo(order.getOrderNo());//向每一个订单详情中追加新生成的订单编号
        }
        int flags = itemDao.batchInsertItems(itemList);
        if(flags < 1){
        ...
        List<Item> itemListResult = itemDao.selectAllByUseridAndOrderno(userId,order.getOrderNo());
    ...
    public ServerRes cancelOrder(Integer userId, Long orderNo) {
        ...
        int flag = orderDao.updateByPrimaryKeySelective(now);
        return flag>0? ServerRes.success(Result.RESULT_SUCCESS): ServerRes.error(Result.RESULT_ERROR);
    }
    ...
    public ServerRes productsPreview(Integer userId) {
        ...
        for(ItemVO itemVO:itemVOList){
            totlePrice = BigDecimalUtil.add(totlePrice,itemVO.getTotalPrice());
        }
    ...
    public ServerRes<OrderVO> getDetailByOrderNo(Integer userId, Long orderNo) {
        ...
        List<Item> itemList = itemDao.selectAllByUseridAndOrderno(userId,orderNo);
        OrderVO orderVO = assembleOrderVO(order,itemList);
        Shipping shipping = shippingDao.selectByPrimaryKey(order.getShippingId());
        orderVO.setReceiverName(shipping.getReceiverName());
        ShippingVO sv = assembleShippingVO(shipping);
    ...
    @Override
    public ServerRes<OrderVO> getDetailByOrderNo(Long orderNo) {
        Order order = orderDao.selectByOrderno(orderNo);
        if(order == null){
            return ServerRes.error(Result.ORDER_NOT_FOUND);
        }
        List<Item> itemList = itemDao.selectAllByOrderNo(orderNo);
        OrderVO orderVO = assembleOrderVO(order,itemList);
        Shipping shipping = shippingDao.selectByPrimaryKey(order.getShippingId());
        orderVO.setReceiverName(shipping.getReceiverName());
        ShippingVO sv = assembleShippingVO(shipping);
    ...
    public ServerRes<PageInfo> list(Integer userId, Integer page, Integer size) {
        ...
        for(Order order:orderList){
            List<Item> itemList = itemDao.selectAllByUseridAndOrderno(userId,order.getOrderNo());
            OrderVO orderVO = assembleOrderVO(order,itemList);
